...
| `app/templates/upload.html` | TinyMCE 主脚本、语言包和自动加载资源增加统一 `cache_suffix` | 避免升级 vendor 后浏览器继续使用旧缓存 | 更新 vendor 时必须同步 manifest 版本 |
| `app/templates/upload.html` | 强制 TinyMCE 外壳纵向 flex、编辑容器撑满、编辑区最小高度 360px | 修复 TinyMCE 内联 `display:block` 导致 iframe 高度 0、无法点击粘贴 | 仅作用于上传页富文本编辑器 |
| `app/__init__.py` | 注入 TinyMCE vendor manifest 读取函数 | 模板从 manifest 获取当前 `asset_version` | manifest 缺失时回退到默认版本 |
| `assets/vendor/tinymce/` | 新增 TinyMCE 运行资源 | 消除上传页编辑器 CDN 依赖 | 增加约 1.4MB 静态资源 |
| `assets/vendor/tinymce/tinymce-manifest.json` | 记录 TinyMCE 版本、资源版本和插件集合 | 作为缓存失效和维护清单来源 | 版本不更新会导致缓存继续命中 |
| `tests/test_social_publish.py` | 增加上传页资源回归测试 | 防止重新引入 CDN 和遗漏 cache busting | 测试位置后续可拆分 |

## 测试策略

- 单测：上传页 HTML 不包含 jsDelivr TinyMCE，包含本地 TinyMCE 资源和 `cache_suffix`，本地资源与 manifest 路由返回 200。
- 语法检查：`python3 -m py_compile app/uploader.py app/__init__.py`。
- 浏览器 smoke：使用临时 admin session 打开本地 `/admin/upload`，等待 `.tox-tinymce` 出现，检查无 jsDelivr 请求、无失败请求、`.tox-edit-area` 和 iframe 高度大于 0，并验证可写入内容。
