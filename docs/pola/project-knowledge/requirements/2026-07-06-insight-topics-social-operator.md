...
- 本次不接入新的大模型生成服务。
- 本次不改变自动刷新频率，不引入需要密钥、浏览器解析或新后台进程的数据采集方式。
- 本次不改 PolaNews 上游新闻数据结构。
- 本次不部署新的队列、cron 或 worker。
- 首轮设计阶段不直接发布生产；2026-07-06 晚用户明确要求“更新线上”后，发布证据记录在 release/devlog 中。
...
- A8 数据源结构：公开数据源不只覆盖新闻与模型发布，还要覆盖企业采用、云端实践、开发者最佳实践、商业模式、投资/创业观察和静态行业实践源。
- A9 来源护栏：新增实时源必须是可机器读取的公开 RSS/Atom 或既有 API；新增行业实践底座源必须是稳定公开 URL 和本地元数据，不新增模型调用、后台进程、密钥或抓取频率；每个源必须有标签和 `SOURCE_LABELS` 显示名或 `source_label`。
- A10 查询意图：PolaNews 与社区查询词应优先面向场景落地、产品能力、商业模式、最佳实践、企业采用、评估护栏和上下文工程；静态行业实践源应补足非新闻、长期有效的实践材料，避免把宽泛新闻词直接放大为运营选题。
