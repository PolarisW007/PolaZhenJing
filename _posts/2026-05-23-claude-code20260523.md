...
theme: claude
title: "你让 Claude Code 写功能、补测试、格式化文件，最后顺手提交"
date: 2026-05-23
tags: [coding-tools, claude, prompt-engineering, transformer, guide, opinion]
summary: "你让 Claude Code 写功能、补测试、格式化文件，最后顺手提交。它功能写了，测试也补了大半，格式化跑了两个文件，但偏偏漏了一个。然后它很自然地告诉你：「搞完了。」你一看：没提交，格式也没完全对齐。但这些问题，其实不该靠你事后检查。在 Claude Code 里，它们都可以配置成自动流程。这就是 Hook 存在的理由。"
description: "用 Claude Code Hook 把写功能、补测试、格式化和提交前检查变成自动流程，减少事后人工兜底。"
image: "/assets/images/generated/claude-code20260523/cover.png"
---

![程序员坐在工作台前，周围环绕着多个自动化齿轮和流水线场景，代表 hooks 将 Claude 的行为抽离并自动化]({{ site.baseurl }}/assets/images/generated/claude-code20260523/cover.png)

# 你让 Claude Code 写功能、补测试、格式化文件，最后顺手提交

你让 Claude Code 写功能、补测试、格式化文件，最后顺手提交。

它功能写了，测试也补了大半，格式化跑了两个文件，但偏偏漏了一个。
...
![代码文件自动流入多个处理站，象征 hooks 将行为配置成自动流程，不再依赖人工检查]({{ site.baseurl }}/assets/images/generated/claude-code20260523/scene-1.png)
...
Claude Code hooks 是在 Claude 生命周期的固定节点自动执行的 shell 命令。

不需要你给 Claude 下任何指令，Hook 就已经跑完了。
...
GitButler 的指南：从三个开始。PostToolUse 自动格式化、PreToolUse 拦截危险命令、Notification 桌面通知，覆盖最广，上手最容易。

---

## 先看这 5 个生命周期事件

Claude Code 有 20 多个生命周期事件，SessionStart 到 SessionEnd。不用全记。下面 5 个是实际工作中真会用的：
...
![Claude 完成回复后，自动触发测试跑道和 Git 提交流水线，象征 Stop 钩子的自动执行]({{ site.baseurl }}/assets/images/generated/claude-code20260523/scene-2.png)
...
![交互菜单界面以机械仪表盘形式呈现，开发者通过旋钮选择事件、设置匹配器]({{ site.baseurl }}/assets/images/generated/claude-code20260523/scene-3.png)
...
**Stdout** 会被加入 Claude 的上下文中，适用于 SessionStart、UserPromptSubmit 这些 hooks。
...
- 其他退出码 → 继续执行，stderr 会被记录下来，可以通过 `Ctrl+O` 查看

![代码在检查站面临分叉路口，继续或阻止的决策由环境机制控制]({{ site.baseurl }}/assets/images/generated/claude-code20260523/scene-4.png)
...
**第三步：打开 `/hooks` 翻一翻交互菜单**

列出全部事件和说明。想想你的工作流里还有哪些可以自动化。Stop 事件（Claude 完成后跑测试）和 PreToolUse（拦截特定模式）是下一个选择。
...
![规则框架像坚固的机械结构强制执行关键动作，象征环境机制而非 prompt 约束]({{ site.baseurl }}/assets/images/generated/claude-code20260523/scene-5.png)
